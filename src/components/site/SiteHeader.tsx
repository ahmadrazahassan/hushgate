...
export function SiteHeader({ installHref }: { installHref: string }) {
  ...
  return (
    <header className="pointer-events-none fixed inset-x-0 top-0 z-50 flex justify-center px-3">
      ...
      >
        <div className="flex h-[60px] flex-nowrap items-center gap-2 pr-2.5 pl-3 md:pl-3.5">
          <Link href="/" className="flex shrink-0 items-center gap-2.5 rounded-xl pr-2" aria-label="Hushgate home">
            <AppTile size={36} />
            <span className="font-display text-[19px] font-bold tracking-[-0.03em]">Hushgate</span>
          </Link>

          <nav className="ml-auto hidden shrink-0 items-center md:flex" aria-label="Main">
            {links.map((link) => (
              <Link key={link.href} href={link.href} className="rounded-full px-3 py-2 text-[15px] font-medium tracking-[-0.01em] whitespace-nowrap text-white/80 transition-colors hover:text-white">
                {link.label}
              </Link>
            ))}
          </nav>

          <Link href={signedIn ? "/account" : "/login"} className="ml-auto hidden shrink-0 items-center gap-1.5 rounded-full px-3 py-2 text-[15px] font-medium tracking-[-0.01em] whitespace-nowrap text-white/80 transition-colors hover:text-white md:ml-2 md:inline-flex">
            {signedIn && <Icon name="user" className="size-[18px]" />}
            {signedIn ? "My account" : "Sign in"}
          </Link>
          <Link
            href={installHref}
            className="inline-flex h-10 shrink-0 items-center gap-2 rounded-[12px] bg-white px-4 text-[15px] font-semibold tracking-[-0.01em] whitespace-nowrap text-[#0b0d12] transition-transform duration-200 hover:scale-[1.03] active:scale-[0.98] max-md:ml-auto md:ml-2"
          >
